...
async function hijackCommand(cmd) {
  ...
  console.log(
    `✅ Found "${cmd}" path: ${checkCmd.stdout.trim().split("\n")[0]}`
  );
  ...
  if (platform === "win32") {
    ...
    try {
      ...
      console.warn(
        `⚠️ There is no shell config file, ready to create new: ${shellConfigPath}`
      );
    }
    ...
    try {
      ...
      console.log(
        `👉 Please reopen your terminal or execute: source ${shellConfigPath}`
      );
    } catch (e) {
